...
export async function POST(request: Request) {
    try {
        ...
        const getBudgetText = (budgetCode: string) => {
            switch(budgetCode) {
                case 'small': return '$1,000 - $5,000';
                case 'medium': return '$5,000 - $10,000';
                case 'large': return '$10,000+';
        ...
        try {
            ...
            const notificationText = NOTIFICATION_TEMPLATE
                .replace(/{{name}}/g, name)
                .replace(/{{email}}/g, email)
                .replace(/{{budget}}/g, budgetText)
                .replace(/{{message}}/g, message);
                
            const autoReplyText = AUTO_REPLY_TEMPLATE
                .replace(/{{name}}/g, name);
